...
def mangleMAC(addr):
    global zero_mac
    if zero_mac: return "\x00\x00\x00\x00\x00\x00"
    if addr in emap: return emap[addr]
    haddr=None
    if addr == "\x00\x00\x00\x00\x00\x00": haddr=addr #return null MAC
    if ord(addr[0])&0x01: haddr=addr #mac&0x800000000000 == broadcast addr, do not touch
    if not haddr:
        haddr=hashaddr(addr)
        haddr=chr(ord(haddr[0])&0xfc|0x2)+haddr[1:6] #return hash bytes with first byte set to xxxxxx10 (LAA unicast)
    emap[addr]=haddr
    return haddr
...
def mangleIP(addr,*ports): #addr,extra=our_port,other_port
    ...
    haddr=None
    intip=iptoint(addr)
    if len(addr)==4 and intip>=0xE0000000: haddr=addr #pass multicast 224.x.x.x and higher
    ip=iptoa(addr)
    ...
    if not anon_all and (ip.startswith('127.') \
                ...
                or ip.startswith('169.254.')): haddr=addr
    #pass ips matching exclude
    for x in exclude:
        if ip.startswith(x): haddr=addr
    if ports and ports[0] in exclude_port: haddr=addr #if our port is exclude
    if not haddr:
        if by_flow: haddr=hashaddr(addr,*ports) #use ports if by flow, else just use ip
        else: haddr=hashaddr(addr)
    return haddr

def mangleIPs(src,dst,sport,dport):
    if by_flow: #if by flow, hash addresses with s/d ports
        if (src,sport,dst,dport) in ipmap: src,dst=ipmap[(src,sport,dst,dport)]
        elif (dst,dport,src,sport) in ipmap: dst,src=ipmap[(dst,dport,src,sport)] #make sure reverse flow maps same
        else: src,dst=ipmap.setdefault((src,sport,dst,dport),(mangleIP(src,sport,dport),mangleIP(dst,dport,sport)))
    else:
        if src in ipmap: src=ipmap[src]
        else: src=ipmap.setdefault(src,mangleIP(src,sport))
        if dst in ipmap: dst=ipmap[dst]
        else: dst=ipmap.setdefault(dst,mangleIP(dst,dport))
    return src,dst
...
if __name__ == '__main__':
    ...
    for f in args[0:-1]:
        p=0
        cap=pcap.pcap(f)
        while cap.dispatch(1,pcap_handler): p+=1 #process whole file
        del cap
        print '%s,%s'%(f,p)
    out.close()

    print "#type,is-anonymized, original, anonymized"
    for ia,oa in sorted(emap.items()): print 'ether,%d, %s, %s'%(int(not ia==oa),mactoa(ia),mactoa(oa))
    for ia,oa in sorted(ipmap.items()):
        ...
        else: print 'ip,%d, %s, %s'%(int(ia!=oa),iptoa(ia),iptoa(oa))
