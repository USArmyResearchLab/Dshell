import urllib,sys,time,socket,struct,datetime
...
def decode_base64(intext, alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/', padchar = '=', debug=False):
	...
	while i < len(intext)-3:
		if intext[i] not in alphabet or intext[i+1] not in alphabet or intext[i+2] not in alphabet or intext[i+3] not in alphabet:
				if debug: sys.stderr.write("Non-alphabet character found in chunk: %s\n" % (hexPlusAscii(intext[i:i+4])))
				if debug: sys.stderr.write("Input: %s" % hexPlusAscii(intext))
				raise Exception
...
def printableText(intext, onlyText = False):
	if onlyText:
		FILTER_std_display=''.join([ chr(x) if x in range(32,127) else '.' for x in range(256) ])
	else:
		FILTER_std_display=''.join([ chr(x) if x in [9,10,13]+range(32,127) else '.' for x in range(256) ])
...
#
UNICODE_CONTROL_CHARS = [ unichr(x) for x in range(0,9)+[11,12]+range(14,0x20)+[0x7f]+range(0x80,0xA0) ]
...
def hexPlusAscii(data, width = 16, offset = 0):
	FILTER_hex_display=''.join([(len(repr(chr(x)))==3) and chr(x) or '.' for x in range(256)])
	dlen = len(data)
	output = ''
	for i in xrange(0, dlen, width):
		...
		output += "%08X   %-*s   %s\n" % (i+offset, 16*3 + 1, hexa, printable)
	return output
...
def strtok(data,sep=',',kvsep='=',as_list=False):
	...
	for p in data.split(sep):
		...
		else: params.append(p.strip())
	if not as_list:
		if not len(params): params=None
		elif len(params)==1: params=params[0]
...
def getopts(args,options,
	...
	for o,v in opts:
		if o not in optd: #new option
			if list_options and o in list_options: optd[o]=[v] #this option wil be a list
			else: optd[o]=v #else set to value
		elif type(optd[o])==list: optd[o].append(v) #append this value to list
		elif auto_list: optd[o]=[optd[o],v] #else make value into a list
		elif replace_value: optd[o]=v #replace value if desired
	return optd,args
