...
@author: tparker
'''
import os,sys,logging,struct,datetime
import dshell,util
...
class Output(object):
    ...
    _FILTER_EXTRA=False #true if you want to remove extra fields from the parsed record
    ...
    def __init__(self,*a,**kw):
        ...
        if 'pcap' in kw: self.pcapwriter=PCAPWriter(kw['pcap'])
        else: self.pcapwriter=None
        ...
        if 'session' in kw: self.sessionwriter=SessionWriter(**kw)
        else: self.sessionwriter=None
    ...
    def setformat(self,formatstr=None,typemap=None):
        ...
        (note this breaks compatibility with text formatting,
          ...
          a typemap of [intype]=outtype (or [in]=(newintype,outtype)
           ...
           '''
        if formatstr: self.format=formatstr+"\n"
        ...
        e=0
        while True:
            s=self.format.find('%',e)+1 #find the next format spec of %(...)
            if s<1 or self.format[s]!='(': break #not %(...
            e=self.format.find(')',s)
            if e<0: break #didn't find a closing paren
            fname=self.format[s+1:e] #get text between parens as field name
            fspec='' #len/precision specs will be 0-9 between ) and type char
            for i in xrange(e+1,len(self.format)):
                if self.format[i] in '1234567890.+-# lLh': fspec+=self.format[i]
                else: break #this char is not a spec char, it is the type char
            ftype=self.format[i]
            i+=1
            if i<len(self.format) and self.format[i]==':': #is the field type a intype:outtype def?
                e=self.format.find(' ',i) #find the end whitespace
                ftype,outtype=self.format[i-1:e].split(':') #split on: to get input:output mapping
            else: outtype=None #output will be same as input type
            e=i #start at next char on loop
            try: #field name to column mapping
                fname,fmap=fname.split(':')
            except: fmap=fname #no mapping
            if typemap and ftype in typemap and not outtype:
                try: (ftype,outtype)=typemap[ftype]
                except: outtype=typemap[ftype]
            self.fields.append((fname,ftype,fspec)) #append the field name,type,spec,mapping
            self.fieldnames.append(fname)
            if outtype: self.fieldmap[fname]=(fmap,outtype) #map of in to out,type
    ...
    def parse(self,*args,**kw):
        ...
        for ts in [k for k in kw if k=='ts' or k.endswith('time')]:
            ...
            kw[dt]=datetime.datetime.fromtimestamp(float(kw[ts])).strftime(self.timeformat) #format properly
        if kw.get('direction') is 'cs': kw['dir_arrow']='->'
        elif kw.get('direction') is 'sc': kw['dir_arrow']='<-'
        else:  kw['dir_arrow']='--'
        if 'format' in kw:
        ...
        if self._FILTER_EXTRA:
            rec.update(dict((f,kw[f]) for f in self.fieldnames if (f in kw and kw[f]!=None)))
            #place extra datadict keys into the extra field (and exclude the addr tuple)
            if self.extra: rec['extra']=self.delim.join(['%s=%s'%(f,kw[f]) for f in sorted(kw.keys()) if f not in self.fieldnames and f != 'addr'])
        else: #not filtering extra, just lump them in as fields
            rec.update(kw)
        #populate the data field
        if args: rec['data']=self.delim.join(map(str,args))
        return rec
    ...
    def dump(self,pkt=None,**kw): #pass packets to pcap
        ...
        if self.pcapwriter: self.pcapwriter.write(pktlen,pktdata,kw['ts'])
        else: self.log(util.hexPlusAscii(str(pkt)),level=logging.DEBUG)
    ...
    def dispatch(self,m,*args,**kwargs):
        '''dispatch from Q pop'''
        if m=='write': self.write(*args,**kwargs)
        if m=='alert': self.alert(*args,**kwargs)
        if m=='dump': self.dump(*args,**kwargs)
    ...
    def __init__(self,*args,**kw):
        ...
        if f:
            ...
            if mode=='noclobber':
                ...
                except OSError: pass #file not found
            self.fh=open(f,mode)
        else: self.fh=sys.stdout

    def write(self,obj,**kw):
        '''write session data to the session output or stdout'''
        if self.sessionwriter: self.sessionwriter.write(obj,**kw)
        elif self.fh: self.fh.write(str(obj))

    def close(self):
        '''close output if not stdout'''
        if self.fh != sys.stdout: self.fh.close()
        Output.close(self)
...
class TextOutput(FileOutput):
    ...
    def __init__(self,*args,**kw):
        if 'extra' in kw: self._DEFAULT_FORMAT += " [ %(extra)s ]"
    ...
    def alert(self,*args,**kw):
        ...
        if rec: self.fh.write(self.format%rec)
...
class DBOutput(Output):
    ...
    _TYPEMAP={ 's':'VARCHAR', 'r':'VARCHAR', 'c':'CHAR(1)',
                       ...
                       'f':'DECIMAL', 'F':'DECIMAL',
                       'g':'DECIMAL', 'G':'DECIMAL',
                       'b':('d','BOOLEAN'), #'b' isn't a python type, so (ftype,DBTYPE) tuple for value formats input as ftype
                       't':('f','TIMESTAMP'), 'D':('s','DATETIME'), #not standard across database types!
                       'A': ('s','INET'), 'H':('s','HOST'), 'N':('s','CIDR') ,'M':('s','MACADDR') } #these are postgres specific

    #acceptable params to pass to db module connect method
    _DBCONNPARAMS=['host','user','passwd','password','db','database','port','charset']
    ...
    def __init__(self,*args,**kw):
        ...
        #if we were passed a config.ini file, parse it and add the k/v pairs to the config
        ...
        self.dbconn=self.db.connect(*args,**dict((k,self.dbconfig[k]) for k in self._DBCONNPARAMS if k in self.dbconfig))
        #do the base init last to catch the format string, etc.. (as it may have come from the config file)
        Output.__init__(self,*args,**self.dbconfig)

    def createtable(self,table=None):
        '''creates a table based on the format string'''
        if not table and 'table' in self.dbconfig: table=self.dbconfig['table']
        try:
            cursor =  self.dbconn.cursor()
            sqlfields=[]
            for fname,ftype,fspec in [f for f in self.fields if f[1]!='T']:
                ctype=self.fieldmap[fname][1]
                #if no width spec, use TEXT instead of VARCHAR and hope the db likes it
                if ctype=='VARCHAR' and not fspec: ctype='TEXT'
                fdef=self.fieldmap[fname][0]+' '+ctype
                if fspec: fdef+='('+fspec.strip('+-# lLh').replace('.',',')+')' #try to conver python format spec to something SQL will take
    ...
    def setformat(self,formatstr=None):
        ...
        ph=self.dbconfig.get('insert_param',
                                                        self._DBTYPE_PLACEHOLDER_MAP.get(self.dbconfig['dbtype'],'%%s')
                                                        )
        ...
        for fname,ftype,fspec in self.fields:
            if ftype=='T': self.tablefield=fname
        sqlfields=[self.fieldmap[fname][0] for (fname,ftype,fspec) in self.fields if fname in self.fieldmap]
        self.insertsql='INSERT INTO "%%s" (%s) VALUES (%s)'%(','.join(sqlfields), ','.join([ph]*len(sqlfields)) )
    ...
    def insert(self,rec,table=None):
        ...
        if not table:
            if 'table' in self.dbconfig: table=self.dbconfig['table']
            elif rec[self.tablefield]: table=rec[self.tablefield]
        try:
            sqlvalues=[]
            cursor = self.dbconn.cursor()
            for fname,ftype,fspec in self.fields:
                if fname in self.fieldmap:
                    if ftype in self.format_types: #do we preformat this data?
                        sqlvalues.append(('%'+fspec+ftype)%rec[fname])
                    else: sqlvalues.append(rec[fname])
            sql=self.insertsql%table #create a INSERT INTO table (fields) VALUES (?,?,?) for execute
...
class PCAPWriter(FileOutput):
    ...
    def __init__(self,*args,**kw):
        FileOutput.__init__(self,*args,**kw)
        if self.fh: self.fh.write(struct.pack('IHHIIII',0xa1b2c3d4,2,4,0,0,65535,1))

    #overrides Output.write to write session as PCAP
    #data flow is Output.dump->pcapwriter.write
    def write(self,pktlen,pktdata,ts):
        if self.fh:
            self.fh.write(struct.pack('II',int(ts),int((ts-int(ts))*1000000)))
            self.fh.write(struct.pack('II',len(pktdata),pktlen)) #captured length, original length
...
class SessionWriter(Output):
    ...
    def write(self,obj,**kwargs):
        out=None
        kw=dict(**kwargs)
        try: # if a session object with info() and data() methods (conn or blob, but not packet)
            kw.update(**obj.info()) #get object info
            kw=self.parse(**kw)
            if self.dir=='both': ds=[None]
            elif self.dir=='split': ds=['cs','sc']
            else: ds=[self.dir]
            for d in ds:
                kw.update(direction=d if d else 'both') #set direction
                out=FileOutput(self.file%kw,mode=self.mode)#format filename and open
                out.fh.write(obj.data(direction=d)) #write obj data for direction
                out.close()
        except:#if not a session object
            out=FileOutput(self.file%kw,mode=self.mode) #build filename from kw
