...
`       use group=clientip,serverip for grouping by clientip,serverip
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.groups={}
        if self.group: self.group=self.group.split('/')
        # Call parent init
    ...
    def alert(self,*args,**kw):
        if self.group:
            ...
            if k not in self.groups:
                r=k[::-1]
                if r in self.groups: k=r #is other dir in groups
                else: self.groups[k]=[]
            self.groups[k].append(kw)
        else: self.__alert(**kw) #not grouping, just print it
    ...
    def close(self):
        ...
        if self.group:
            for k in sorted(self.groups.iterkeys()):
                #write header
                self.fh.write(' '.join( '%s=%s'%(self.group[i],k[i]) for i in xrange(len(self.group)))+'\n')
    ...
    def __alert(self,*args,**kw):
        self.fh.write('%s  %16s -> %16s  (%s -> %s) %4s  %6s  %6s %5d  %5d  %7d  %7d  %-.4fs\n' % ( datetime.datetime.utcfromtimestamp(kw['starttime']),
                                                                kw['clientip'],
                                                                kw['serverip'],
                                                                kw['clientcountrycode'],
                                                                kw['servercountrycode'],
                                                                kw['proto'],
                                                                kw['clientport'],
                                                                kw['serverport'],
                                                                kw['clientpackets'],
