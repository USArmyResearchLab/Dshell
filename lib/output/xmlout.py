...
class XMLOutput(output.FileOutput):
    ...
    def __init__(self,*args,**kwargs):
        ...
        output.FileOutput.__init__(self,*args,**kwargs) #pass all to fileoutput
    ...
    def alert(self,*args,**kwargs):
        ...
        self.element=ET.SubElement(self.root,'alert',self._filter_attr(kwargs))
    ...
    def write(self,obj,parent=None,**kwargs):
        ...
        if not parent: parent=self.element
        kw=dict(**kwargs)
        tag=str(type(obj)).split("'",2)[1] # turns "<xxxx 'yyyy'>" into "yyyy"
        if tag.startswith('dshell.'):  #is a dshell object
            kw.update(**obj.info()) #get attribs
            tag=tag.split('dshell.')[1] # turns "dshell.Connection" into "Connection"
        e=ET.SubElement(parent,tag,self._filter_attr(kw))
        if tag=='Connection': #recurse on blobs in conn
            for blob in obj: self.write(blob,parent=e)
            return #subobjects will have the data
        e.text=self._filter_text( str(obj) ) #leave this up to the object to handle

    def _filter_attr(self,d): return dict((k,str(v)) for (k,v) in d.iteritems())
