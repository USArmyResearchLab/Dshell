...
class DNSDecoder(dshell.TCPDecoder):
    ...
    def blobHandler(self,conn,blob):
        ...
        for data in blob: #iterate blob as each packet will be a seperate request (catches spoofing)
            ...
            if dns.qr == dpkt.dns.DNS_Q:
                ...
                if "DNSHandler" in dir(self):
                    self.DNSHandler(conn=conn,request=rq[1],response=dns,requesttime=rq[0],responsetime=blob.starttime,responsecount=rq[2])
        ...
        if self.noanswer and "DNSHandler" in dir(self) and self.requests.get(conn):
            for requesttime,request,responsecount in self.requests[conn].values():
                if not responsecount:
                    self.DNSHandler(conn=conn,request=request,response=None,requesttime=requesttime,responsetime=None,responsecount=responsecount)
