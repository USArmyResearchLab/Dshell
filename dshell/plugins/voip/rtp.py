...
Output:
  ...
  ''',
    ...
    def packet_handler(self, pkt):
        ...
        self.write(f'\n\tFrom: {pkt.sip} ({pkt.smac}) to {pkt.dip} ({pkt.dmac}) \n\tPayload Type (7 bits): {pt}\n\tSequence Number (16 bits): {rtpp.seq}\n\tTimestamp (32 bits): {rtpp.ts} \n\tSynchronization source (32 bits): {rtpp.ssrc}\n\tArrival Time: {pkt.ts} --> {datetime.datetime.utcfromtimestamp(pkt.ts)}\n\tContributing source (32 bits): {rtpp.cc}, Padding (1 bit): {rtpp.p}, Extension (1 bit): {rtpp.x}, Marker (1 bit): {rtpp.m}\n', **pkt.info())
