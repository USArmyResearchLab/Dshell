...
class DshellPlugin(HTTPPlugin):
    ...
    def http_handler(self, conn, request, response):
        ...
        if self.maxurilen > 0 and len(uri_location) > self.maxurilen:
            uri_location = f'{uri_location[:self.maxurilen]}[truncated]'
            pretty_url = host + uri_location

        # Set the first line of the alert to show some basic metadata
        if response == None:
            msg = [f'{request.method} (NO RESPONSE) {pretty_url}']
        else:
            msg = [f'{request.method} ({response.status}) {pretty_url} ({response.headers.get('content-type', '[no content-type]')})']
        ...
        if post_params:
            ...
            if self.maxpost > 0 and len(request.body) > self.maxpost:
                msg.append(f'{request_body[:self.maxpost]}[truncated]')
            else:
