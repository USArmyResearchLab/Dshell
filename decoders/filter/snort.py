...
""",
    ...
    def preModule(self):
        ...
        if self.conf: #
            ...
            if not self.rule or not len(self.rule): self.warn("No rule specified (--%s_rule)" % self.name)
            else: rules=[self.rule]
        self.rules=[]
        for r in rules:
            try: self.rules.append( ( self.parseSnortRule(r) ) )
            except Exception,e:
                self.error('bad snort rule "%s": %s'%(r,e))
                if self._DEBUG: self._exc(e)
        if self.subDecoder: self.subDecoder.ignore_handshake=True #we filter individual packets so session-based subdecoders will need this set
        dshell.IPDecoder.preModule(self)

    def rawHandler(self,pktlen,pkt,ts,**kwargs):
        kwargs['raw']=pkt #put the raw frame in the kwargs
        return dshell.IPDecoder.rawHandler(self,pktlen,pkt,ts,**kwargs) #continue decoding

    def IPHandler(self,addr,pkt,ts,**kwargs):
        '''check packets using filterfn here'''
        raw=str(kwargs['raw']) #get the raw frame for forwarding if we match
        ...
        for r,msg in self.rules:
            if r(p):  #if this rule matched
                match=True
                if msg: a.append(msg) #append rule message to alerts
                if self.none or not self.all: break #unless matching all, one match does it
            else:  #last rule did not match
                match=False
                if self.all: break #stop once no match if all
        ...
        if (match is not None) and ((match and not self.none) or (self.none and not match)):
            ...
            self.count+=1
            if self.alerts: self.alert(*a,**p.info())
            if self.subDecoder: self.subDecoder.decode(len(raw),raw,ts) #decode or dump packet
            else: self.dump(len(raw),raw,ts)
    ...
    def parseSnortRule(self,ruletext):
        ...
        KEYWORDS=('msg','content') #rule start, signal when we process all seen keywords
        msg=''
        f=[]
        rule=ruletext.split(' ',7)
        (a,proto,sip,sp,arrow,dip,dp)=rule[:7]
        if len(rule)>7: rule=rule[7]
        else: rule=None
        if a != 'alert': raise Exception('Must be alert rule')
        f.append('p.proto == "'+proto.upper()+'"')
        if sip != 'any': f.append('p.sip == "'+sip+'"')
        if dip != 'any': f.append('p.dip == "'+dip+'"')
        if sp != 'any': f.append('p.sport == '+sp)
        if dp != 'any': f.append('p.dport == '+dp)
        f=['(' + (' and '.join(f)) + ')' ] #create header condition
        if rule: rule=rule.strip('()').split(';') #split between () and split on ;
        last=None #no last match
        while rule:
            try: k,v=rule.pop(0).strip().split(':',1)
            except: continue
            if k.lower() == 'content': #reset content match
                content=v.strip().strip('"')
                if content.startswith('|') and content.endswith('|'): #hex bytes?
                    content=''.join( '\\x'+c for c in content.strip('|').split() )
                nocase=depth=offset=distance=within=None
                while rule:
                    r=rule[0].strip()
                    if ':' in r: k,v=r.split(':',1)
                    else: k,v=r,None
                    k=k.lower()
                    if k in KEYWORDS: break #next rule part
                    elif k == 'nocase': nocase=True
                    elif k == 'depth': depth=int(v)
                    elif k == 'offset': offset=int(v)
                    elif k == 'distance': distance=int(v)
                    elif k == 'within': within=int(v)
                    rule.pop(0) #remove this keyword:valuea
                #add coerce to lower if nocase?
                if nocase: nocase='.lower()'
                else: nocase=''
                st,end=offset,depth #start,end offsets of find(), maybe number or result of another find()
                if last: #if we have a last content match, use the distance/within kws
                    #within means this match has to be within X from previous+distance, so use previous as offset and within as depth
                    if within: st,end=last,last+'+'+str(within) #set to last match and X from last match
                    #distance means the next match must be AT LEAST X from the last
                    if distance: st=last+'+'+str(distance)# set start to last match+distance
                #else use the offset/depth values as given
                last='p.pkt'+nocase+'.find('+"'"+content+"'"+nocase+','+str(st)+','+str(end)+') != -1'
            if k.lower() == 'msg': msg=v.strip().strip('"')#get alert message
        if last: f.append('('+last+')')
        f=' and '.join(f)
