...
class DshellDecoder(dshell.TCPDecoder):
    def __init__(self):
        ...
        self.chainable=True # sets chainable to true and requires connectionInitHandler() and connectionCloseHandler()

    def preModule(self,*args,**kwargs):
        dshell.TCPDecoder.preModule(self, *args, **kwargs)
        #twp handle hex keys
        self.key=self.makeKey(self.key)
        if self.cskey: self.cskey=self.makeKey(self.cskey)
        if self.sckey: self.sckey=self.makeKey(self.sckey)

    def makeKey(self,key):
        if key.startswith('"'): return key[1:-1]
        if key.startswith('0x'):
            k,key='',key[2:]
            for i in xrange(0,len(key),2): k+=chr(int(key[i:i+2],16))
            return k
        else: return struct.pack('I', int(key))
    ...
    def blobHandler(self,conn,blob):
        ...
        if self.cskey != None and blob.direction == 'cs': key=self.cskey
        elif self.sckey != None and blob.direction == 'sc': key=self.sckey
        else: key=self.key
        for i in xrange(len(data)):
            if self.resync and data[i:i+len(key)]==key: k=0 #resync if the key is seen
            newdata+=chr(ord(data[i])^ord(key[k])) #xor this byte with the aligned byte from the key
            k = (k+1) % len(key) #move key position
        # update our connection object with the new data
        newblob = self.xorconn[conn.addr].update(conn.endtime,blob.direction,newdata)
        ...
        if newblob and 'blobHandler' in dir(self.subDecoder):
            self.subDecoder.blobHandler(self.xorconn[conn.addr],newblob) # pass to the subDecoder's blobHandler()
