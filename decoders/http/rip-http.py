...
class DshellDecoder(HTTPDecoder):
    ...
    def preModule(self):
        if self.content_filter: self.content_filter=re.compile(self.content_filter)
        ...
        if (not self.direction or self.direction=='cs') and request and request.method=='POST' and request.body:
            ...
            if not self.content_filter or self.content_filter.search(response.headers['content-type']):
                ...
                if self.openfiles[url].done():
                    self.alert("File done: %s (%s)" % (self.openfiles[url].filename, url), conn.info())
...
class httpfile:
    ...
    def __init__(self,filename):
        ...
        self.ranges = []       # List of tuples indicating byte chunks already received and written to disk
    ...
    def __del__(self):
        ...
        if not self.done():
            ...
            try: os.rename(self.filename, self.filename+"_INCOMPLETE")
            except: pass
            ls = 0
            le = 0
    ...
    def handleresponse(self,response):
        ...
        if 'content-range' in response.headers:
            m = re.search('bytes (\d+)-(\d+)/(\d+|\*)', response.headers['content-range'])
            if m:
                range_start = int(m.group(1))
                range_end = int(m.group(2))
                if len(response.body) < (range_end-range_start+1): range_end=range_start+len(response.body)-1
                try:
                    if int(m.group(3)) > self.size: self.size = int(m.group(3))
                except: pass
        elif 'content-length' in response.headers:
            try:
                if int(response.headers['content-length']) > self.size: self.size = int(response.headers['content-length'])
            except: pass
    ...
    def checkranges(self):
        ...
        #print self.ranges
        for s,e in self.ranges:
            if s <= current_end+1: current_end = e
            else:
                foundgap = True
                current_start = s
                current_end = e
