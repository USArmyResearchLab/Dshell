                ...
                for an in dns.an:
                    if an.type == dpkt.dns.DNS_A:
                        try:
                            ...
                            if self.foreign and ( cc == 'US' or cc == '--' ): continue
                            elif self.code != None and cc != self.code: continue
                            answers.append('A: %s (%s) (ttl %ss)' % (socket.inet_ntoa(an.ip), cc, an.ttl) )
                        except: continue
                    elif an.type == dpkt.dns.DNS_AAAA:
                        try:
                            cc = self.getGeoIP( socket.inet_ntop(socket.AF_INET6, an.ip6) )
                            if self.foreign and ( cc == 'US' or cc == '--' ): continue
                            elif self.code != None and cc != self.code: continue
                            answers.append('AAAA: %s (%s) (ttl %ss)' % (socket.inet_ntop(socket.AF_INET6, an.ip6), cc, an.ttl) )
                        except: continue
                    else:
                        # un-handled type
                        continue
