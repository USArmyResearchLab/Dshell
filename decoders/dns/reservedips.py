...
class DshellDecoder(DNSDecoder):
    ...
    def inReservedSpace(self,ipaddress):
        for net in self.reservednets:
            if ipaddress in net: return True
        return False
            ...
            # DNS Answer with data and no errors
                ...
                for an in dns.an:
                    if an.type == dpkt.dns.DNS_A:
                        try:
                            if self.inReservedSpace(socket.inet_ntoa(an.ip)): answers.append('A: '+socket.inet_ntoa(an.ip)+' (ttl '+str(an.ttl)+'s)')
                        except: continue
                    elif an.type == dpkt.dns.DNS_AAAA:
                        try:
                            if self.inReservedSpace(socket.inet_ntop(socket.AF_INET6,an.ip6)): answers.append('AAAA: '+socket.inet_ntop(socket.AF_INET6,an.ip6)+' (ttl '+str(an.ttl)+'s)')
                        except: continue
                    else:
                        # un-handled type
                        continue
