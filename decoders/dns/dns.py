...
class DshellDecoder(DNSDecoder):
    ...
    def decode_q(self,dns):
        ...
        if dns.qd[0].type == dpkt.dns.DNS_PTR:
            if dns.qd[0].name.endswith('.in-addr.arpa'):
                query_name = '.'.join(reversed(dns.qd[0].name.split('.in-addr.arpa')[0].split('.')))
            else:
                query_name = dns.qd[0].name
            queried = queried + "PTR? %s" % (query_name)

        if not self.showall: return queried
    ...
    def DNSHandler(self,conn,request,response,**kwargs):
        if self.only_norequest and request is not None: return
        if not self.show_norequest and request is None: return
        anstext=''
        queried=''
        id=None
        for dns in request,response:
            ...
            # DNS Answer with data and no errors
                ...
                for an in dns.an:
                    if an.type == dpkt.dns.DNS_A:
                        try: answers.append('A: %s (ttl %ss)' % (socket.inet_ntoa(an.ip), str(an.ttl)))
                        except: continue
                    elif an.type == dpkt.dns.DNS_AAAA:
                        try: answers.append('AAAA: %s (ttl %ss)' % (socket.inet_ntop(socket.AF_INET6,an.ip6),str(an.ttl)))
                        except: continue
                    elif an.type == dpkt.dns.DNS_CNAME: answers.append('CNAME: '+an.cname)
                    elif an.type == dpkt.dns.DNS_PTR: answers.append('PTR: '+an.ptrname)
                    elif an.type == dpkt.dns.DNS_NS: answers.append('NS: '+an.nsname)
                    elif an.type == dpkt.dns.DNS_MX: answers.append('MX: '+an.mxname)
                    elif an.type == dpkt.dns.DNS_TXT: answers.append('TXT: '+' '.join(an.text))
                    elif an.type == dpkt.dns.DNS_SRV: answers.append('SRV: '+an.srvname)
                    else:
                        # un-handled type
                        continue
                if queried != '':
            ...
            #NXDOMAIN in response
                ...
                if queried != '': anstext='NXDOMAIN'

        if anstext and not self.only_noanswer and not self.only_norequest: #did we get an answer?
            ...
            self.alert(str(id)+' '+conn.query+' / (no answer)',**conn.info())
