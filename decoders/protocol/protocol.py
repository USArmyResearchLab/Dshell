...
try: PROTOCOL_MAP = dict((v,k[9:]) for k,v in dpkt.ip.__dict__.iteritems() if type(v) == int and k.startswith('IP_PROTO_') and k != 'IP_PROTO_HOPOPTS')
except: PROTOCOL_MAP = {}
...
class DshellDecoder(dshell.IPDecoder):
    ...
    def packetHandler(self,ip):
        ...
        self.alert('PROTOCOL: %s (%d)' % (p, ip.proto), sip=ip.sip, dip=ip.dip, ts=ip.ts)
