...
class DshellDecoder(dshell.TCPDecoder):
    def __init__(self):
        dshell.TCPDecoder.__init__(self,
                        ...
                        filter = 'tcp',
                        author = 'bg',
                        optiondict={'size': {'type':'float','default':1,'help':'number of megabytes transferred'}}
                        )
    ...
    def preModule(self):
        if self.size <= 0:
            self.warn("Cannot have a size that's less than or equal to zero. (size: %s)" % (self.size))
            self.size = 1
        self.min = 104857 * self.size
        self.debug("Input: %s,  Final size: %s bytes" % (self.size, self.min))

    def connectionHandler(self,conn):
        if (conn.clientbytes + conn.serverbytes) >= self.min: self.alert(**conn.info())
